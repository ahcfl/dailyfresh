...
# ajax发起的请求都在后台，在浏览器中看不到效果 所以这里不能使用 LoginRequiredMixin(object) 判断是否登录
# /cart/add
...
# 更新购物车记录
# 采用ajax post请求
# 前端需要传递的参数:商品id(sku_id) 更新的商品数量(count) 我们cache到redis中的
# /cart/update
...
class CartDeleteView(View):
    ...
    def post(self, request):
        ...
        # 计算用户购物车中商品的总件数 {'1':5, '2':3}
        total_count = 0
        vals = conn.hvals(cart_key)  # 取hash里所有值
        for val in vals:
            total_count += int(val)
